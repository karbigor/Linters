...
def create_app(test_config=None):
    ...
    with app.app_context():
        ...
        if new:
            start_data = [
                Client(name='Igor', surname='Karbushev',
                       credit_card='7498269', car_number='g345hh'),
                Client(name='Zarek', surname='Danelian',
                       credit_card='9879856', car_number='a951ky'),
                Parking(address='Ленина, 1', opened=True,
                        count_places=35, count_available_places=34),
                Parking(address='Бабушкина, 35', opened=True,
                        count_places=63, count_available_places=62),
                Client_Parking(client_id=2, parking_id=2,
                               time_in=datetime.datetime.now()),
                Client_Parking(client_id=1, parking_id=1,
                               time_in=datetime.datetime.now())]
    ...
    def parkings_post():
        ...
        if 'opened' in data:
            ...
            data['opened'] = False
        new_park = Parking(address=data['address'], opened=data['opened'],
    ...
    def client_parkings_post():
        ...
        added_cli_park = db.session.query(Client_Parking).where(
            Client_Parking.id == new_cli_park.id).first()
    ...
    def client_parkings_delete():
        ...
        park = db.session.query(Parking).where(Parking.id
                                               == data['parking']).first()
        cli_park = db.session.query(Client_Parking).where(
            Client_Parking.parking_id == data['parking'],
        ...
        cli_park = db.session.query(Client_Parking).where(
            Client_Parking.id == cli_park.id).first()
