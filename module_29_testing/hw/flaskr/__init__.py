...
def create_app(test_config=None):
    ...
    with app.app_context():
        ...
        if new:
            start_data = [
                ...
                Parking(
                    address="Бабушкина, 35",
                    opened=True,
                    count_places=63,
                    count_available_places=62,
                ),
                Client_Parking(
                    client_id=2, parking_id=2, time_in=datetime.datetime.now()
                ),
                Client_Parking(
                    client_id=1, parking_id=1, time_in=datetime.datetime.now()
                ),
            ]
    ...
    def clients_post():
        ...
        db.session.commit()
        added_cli = (
            db.session.query(Client).where(Client.id == new_cli.id).first()
        )
    ...
    def parkings_post():
        ...
        db.session.commit()
        added_park = (
            db.session.query(Parking).where(Parking.id == new_park.id).first()
        )
    ...
    def client_parkings_post():
        ...
        added_cli_park = (
            ...
            .where(Client_Parking.id == new_cli_park.id)
            .first()
        )
    ...
    def client_parkings_delete():
        data = request.form.to_dict()
        cli = (
            db.session.query(Client).where(Client.id == data["client"]).first()
        )
        ...
        cli_park = (
            ...
            .where(
                ...
                Client_Parking.client_id == data["client"],
            )
            .first()
        )
        if cli_park:
    ...
    def delmet():
        ...
        responce = requests.delete(
            f"{request.scheme}://{request.host}" f"/client_parkings", data=data
        )
    ...
    def all():
        ...
        return {
            "clients": all_cli_json,
            "parkings": all_park_json,
            "client-parkings": all_park_cli_json,
        }, 200
